package com.edu.ifpb.dac.dacademico.desktop.lookup;

/**
 *
 * @author douglasgabriel
 * @version 0.1
 */
public class LookupPath {

    public static final String ADMIN_SERVICE = "java:global/web/AdminServiceImpl";
    public static final String ALUNO_SERVICE = "java:global/web/AlunoServiceImpl";    
    public static final String CURSO_SERVICE = "java:global/web/CursoServiceImpl";    
    public static final String DISCIPLINA_SERVICE = "java:global/web/DisciplinaServiceImpl";    
    public static final String PROFESSOR_SERVICE = "java:global/web/ProfessorServiceImpl";    
    public static final String TURMA_SERVICE = "java:global/web/TurmaServiceImpl";    
    public static final String AULA_SERVICE = "java:global/web/AulaServicoImpl";    
    public static final String HORARIO_SERVICE = "java:global/web/HorarioServiceImpl";    
    public static final String SALA_SERVICE = "java:global/web/SalaServiceImpl";    
    public static final String REPOSICAO_SERVICE = "java:global/web/ReposicaoAulaServicoImpl";    
    
}
